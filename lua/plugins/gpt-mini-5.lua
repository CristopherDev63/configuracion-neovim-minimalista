...
function M.complete(request)
    ...
    local response = vim.fn.system(string.format("curl -s -X POST -H 'Content-Type: application/json' -d '{\"prompt\": \"%s\"}' http://localhost:5000/completions", vim.fn.escape(prompt, '"')))
