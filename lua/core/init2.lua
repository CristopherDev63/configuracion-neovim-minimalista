...
require("lazy").setup({
		...
		config = function()
			require("nvim-treesitter.configs").setup({
				ensure_installed = {
					...
					"sql",
					"html",
					"css", -- Agregados PHP, SQL, HTML y CSS
				},
				highlight = {
					enable = true,
		...
		config = function()
			require("conform").setup({
				formatters_by_ft = {
					...
					sh = { "shfmt" },
					bash = { "shfmt" },
					php = { "php_cs_fixer" }, -- Agregado formateador para PHP
				},
				format_on_save = {
					format_on_save = nil,
					timeout_ms = 500,
	...
	-- Plugins adicionales para PHP y MySQL
	{
		"phpactor/phpactor",
		ft = "php",
		build = "composer install --no-dev --optimize-autoloader",
		config = function()
			vim.keymap.set("n", "<leader>pm", ":PhpActor context_menu<CR>", { desc = "PHP Actor Context Menu" })
			vim.keymap.set("n", "<leader>pc", ":PhpActor class_new<CR>", { desc = "PHP Actor New Class" })
		end,
	},
	{
...
vim.api.nvim_create_autocmd("FileType", {
	pattern = { "python", "sh", "bash", "php" }, -- Agregado php
	callback = function()
		local filetype = vim.bo.filetype
...
vim.api.nvim_create_autocmd("BufEnter", {
	pattern = { "*.py", "*.sh", "*.bash", "*.php" }, -- Agregado php
	callback = function()
		local filetype = vim.bo.filetype
